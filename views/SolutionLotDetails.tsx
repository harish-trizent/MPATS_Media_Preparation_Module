...
interface SolutionLotDetailsProps {
  lotId: string;
  onBack: () => void;
}

const SolutionLotDetails: React.FC<SolutionLotDetailsProps> = ({ lotId, onBack }) => {
  const distributionData = [
    {
      name: 'SLN-PRIMARY-UNIT-01',
      stockLevel: 82,
      availableStock: 164,
      unit: 'mL',
      firstOpenedDate: 'Jan 15, 2026',
      shelfLife: '11 Months',
      shelfLifeStatus: 'OPTIMAL',
      shelfLifeStatusColor: 'text-emerald-500'
    },
    {
      name: 'SLN-RESERVE-UNIT-02',
      stockLevel: 100,
      availableStock: 200,
      unit: 'mL',
      ...
      <div className="px-12 py-8 bg-white flex items-center justify-between border-b border-slate-100">
        <div className="flex items-center gap-16 flex-1">
          ...
          <div className="space-y-1.5 border-r border-slate-100 pr-12">
            <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Solution Batch Name</p>
            <div className="flex items-center gap-3">
              <span className="material-symbols-outlined text-orange-500 text-xl font-bold">water_drop</span>
              <span className="text-xl font-black text-[#2D3E50] tracking-tight">{lotId || 'SLN-QA-229'}</span>
            </div>
          </div>
          ...
          <div className="space-y-1.5 border-r border-slate-100 pr-12">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-xl font-black text-[#2D3E50] tracking-tight">Sigma-Aldrich Corp.</span>
            </div>
          </div>
          ...
          <div className="space-y-1.5 border-r border-slate-100 pr-12">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-xl font-black text-[#2D3E50] tracking-tight">Dec 12, 2025</span>
            </div>
          </div>
          ...
          <div className="space-y-1.5 pr-12">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-xl font-black text-red-500 tracking-tight">Dec 12, 2026</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="px-8 py-6 flex justify-between items-center border-b border-slate-50 bg-[#FBFDFE]">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-base font-black text-[#2D3E50] tracking-tight">Solution Lot Detailed Distribution</h3>
            </div>
            <span className="text-[11px] font-bold text-slate-300 italic">Last Usage: Today 09:30 AM</span>
          </div>
            ...
            <table className="w-full text-left">
              <thead className="bg-[#F8FAFC] text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100">
                <tr>
                  ...
                  <th className="px-10 py-5">Volume Level</th>
                  <th className="px-10 py-5">Available Volume</th>
                  <th className="px-10 py-5">First Opened Date</th>
                  <th className="px-10 py-5 text-right">Remaining Life</th>
                </tr>
              </thead>
                  ...
                  <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-10 py-8">
                      <div className="flex items-baseline gap-2">
                        <span className={`text-2xl font-black ${lot.availableStock < 50 ? 'text-red-500' : 'text-[#2D3E50]'}`}>
                          {lot.availableStock}
                        </span>
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{lot.unit}</span>
                      </div>
                    </td>
                    <td className="px-10 py-8">
                      {lot.firstOpenedDate ? (
...
export default SolutionLotDetails;
